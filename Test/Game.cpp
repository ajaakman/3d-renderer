...
{	
	...
	p_Renderer->CreateSprite2D("s1", glm::vec2(p_Window->GetWidth() / 2, p_Window->GetHeight() / 2), glm::vec2(70.0f, 70.0f), 1);
	p_Renderer->CreateSprite2D("s2", glm::vec2(p_Window->GetWidth()/2+35, p_Window->GetHeight()/2+35), glm::vec2(70.0f, 70.0f), 1);
...
{ 	
	...
	//p_Renderer->GetRenderable3d("3D")->Rotation = rot;
	//p_Renderer->GetRenderable3d("3D")->Position.x += increment * fDeltaTime * 200;

	p_Renderer->GetSimpleRenderable("r3")->Rotation = rot;
